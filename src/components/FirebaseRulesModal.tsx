  ...
  match /databases/{database}/documents {
    ...
    function isValidUserProfile(data) {
      return data.keys().hasAll(['userId', 'name', 'role', 'onboardingCompleted', 'createdAt'])
        ...
        && (!('subjects' in data) || (data.subjects is list && data.subjects.size() <= 30))
        && (!('latestInsights' in data) || (data.latestInsights is list && data.latestInsights.size() <= 20))
        && (!('latestNextAction' in data) || data.latestNextAction is map);
    }
    ...
    function isValidTransaction(data, userId) {
      return data.keys().hasAll(['userId', 'amount', 'type', 'category'])
        ...
        && data.type in ['expense', 'income', 'savings']
